...
try {
  const stored = JSON.parse(localStorage.getItem("hud_sources"));
  sources = Array.isArray(stored) && stored.length > 0 ? stored : [];
} catch {
  sources = [];
}
...
function render() {
  ...
  if (sources.length === 0) {
    ...
    if (editMode) {
      ...
      msg.style.cssText = "color:rgba(255,255,255,0.4);font-family:sans-serif;font-size:12px;padding:20px;";
      msg.textContent = "Add sources";
...
function save() {
  localStorage.setItem("hud_sources", JSON.stringify(sources));
}
